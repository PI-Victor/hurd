...
def validate_workspace(ctx, param, value):
    ...
    if param.name == 'workspace' and not value:
        if not ('PINNER_WORKSPACE' in os.environ) or os.environ.get('PINNER_WORKSPACE') == '':
            raise click.UsageError("""you need to pass --workspace or export PINNER_WORKSPACE pointing to the full
path directory where the config and platform versioning are located.
            """)
...
_global_options = [
    ...
    click.option(
        ...
        help="""The workspace containing the configuration and the platform versioning in yaml
format.
        """,
...
def get_platform_versions(workspace_path):
    """Will walk the current directory where the application is located and will try to
find all the directories that match a regex of v[0-9]+. e.g.: v1, v2, v2.2

    return: a list of strings that contain the directory names that match the regex.
    """
...
@cli.command(
    help="""This command will start fetching all the repositories defined in a pinned
version that match the specific version found. If multiple pinned versions
match, it will fail with an error.
    """
)
...
@cli.command(
    help="""Displays the pinned microservice versions together with some relevant metadata
such as pinned refs and url. The data is tabulated.
    """
)
@add_option(_global_options)
...
@cli.command(
    help="""This command will validate the defined platform pinned version by trying to
fetch the refs described. If multiple versions match, it will try to validate
all of them.
    """
)
@add_option(_global_options)
